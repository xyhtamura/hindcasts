...
### Metachamber — *the chamber shaped by the future; a tail that knows how much room it has* · ✅ **MVP built → `metachamber/index.html`**
Reverb's original sin: the tail cannot know how much room it has — **mud is a tail colliding with an event it couldn't see.** A live reverb is one room compromised across all events; engineers fix it by riding wet/damping automation, i.e. *manually performed acausality* (they've heard the whole song). Metachamber makes that foreknowledge the effect: read the whole file, build the **gap map** (per event: gap to next onset, next event's level, per-band variant, audibility floor from global stats), fit each event's tail to the silence that actually follows it. Lookahead can't buy this — a limiter looks ahead milliseconds; a tail's budget is *seconds*. **Masking credit** is the second acausal fact: spill under a louder next hit is free, and knowing that requires the future event's *level*, not just its time. Four stances, one engine: **FIT** (every hit gets its own room — RT60 solved per event; no live approximation exists), **DUCK** (fixed room, zero-phase wet ride — the automation ritual automated), **SWELL** (pre-verb with a budget), **HOLLOW** (wet only in the negative space). Room law = **drawn curve** gap→RT60 (inscription, not knob). Absorbs three one-liners: pre-verb, future-sidechain-on-own-wet, and (deferred, as kernel source) self-convolution. **Caesura is suite substrate, not private** — the shared gap-aware mechanic now serves both Metachamber tails and Pythia repeat trains; delay and reverb stay separate tools (engines differ in kind: sparse taps + feedback vs t²-growing echo density through diffusion — the sounder collapse was same-op-different-τ and does not apply here), but the analysis and boundary vocabulary collapse to one module. Superpowers: **precognition + whole-signal optimization + global statistics.** MVP ships FIT + DUCK, a versioned sample-domain gap-map JSON, deterministic three-room FDN rendering, exact-buffer preview/A-B, and float WAV bounce; SWELL/HOLLOW/per-band sensing remain deferred.
...
- **Pre-verb** — acausal reverb whose tail *precedes* each hit; every transient swells into itself. Reverse reverb as principle, not tape trick. *→ absorbed into Metachamber (SWELL stance).*
...
- **The de-LFO** (chorus / flanger / phaser) — the whole modulation family is one causal generator (the LFO) wearing different delay clothes. Replace the oscillator with a curve **drawn or fit against known events** — the flanger's null lands exactly on the downbeat because it read the score. Third appearance of the TaboTa inscription argument (after morph-curve and pitchbend). Superpowers: **precognition** + inscription-of-control. *Coverage note (2026-07-12): chorus is already Pythia (a chorus is a jittered grain cloud — Oracle Chorus preset); flanger is not — it needs **time-varying Time** (the sweep is the LFO), penciled as Pythia's Time-curve extension rather than a new tool. Through-zero flanging is secretly acausal-native: live TZF must delay the dry path too — a hidden lookahead confession — while Pythia's signed Time crosses zero natively.*
...
- **`Metachamber`** — ✅ **MVP built and verified** (`metachamber/index.html`; spec/status in `metachamber/metachamber.md`). The **future-shaped reverb** and first Caesura consumer: versioned whole-file gap map, relative Spill + masking credit, deterministic three-room FDN with event-side FIT routing, fixed-room zero-phase DUCK, wet-only budget guards, exact-buffer preview/A-B, and 32-bit float WAV bounce. Node DSP/schema/WAV/worker-transfer tests pass; a dependency-free Edge lifecycle harness is included. Deferred: SWELL, HOLLOW, room-law drawing, per-band sensing, exact per-event kernels, and per-event hypergeometry.
