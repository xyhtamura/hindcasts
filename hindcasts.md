...
**→ next move for `sounder`: gratuitous multiband.** Right now the RMS window is global — one regime for the whole signal. Split into bands and give **each band its own RMS window** (and own curve). Now the regime is *per-band*: waveshape the highs (sub-ms window, mangle the transient grit) while *compressing* the body (40ms) and *normalizing* the sub (300ms) — selective dynamics where you choose which spectral region gets waveshaped and which gets leveled, independently. The multiband split is the natural carrier for "different timescales of acausality applied to different parts of the signal at once." Gratuitous on purpose — 4, 8, however many bands; the point is that the regime slider becomes a *regime field across frequency*.
...
- **`sounder`** — the compressor member, fused with the histogram waveshaper. Time-occupancy depth chart + redrawable transfer curve; RMS-window-as-regime-slider (waveshape → compression → normalizing).
...
1. **`sounder` → gratuitous multiband** — per-band RMS windows + per-band curves; the regime slider becomes a regime *field* across frequency. Cheapest high-payoff move: it's an extension of a thing already built, and it makes "different timescales of acausality at once" playable. *(do this first.)*
2. **Histogram-waveshaper presets** — the automatic CDF-onto-target warps (uniform / Gaussian / arcsine / another-file's-histogram) as one-click modes inside `sounder`, plus the cross-file version on Pythia's control/source input. Closes the audio↔image histogram loop.
